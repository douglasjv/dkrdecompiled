...
- Generated at: 2026-05-14 23:25:55Z
- Branch: `master`
- HEAD: `01a5914c`
- Completed task: `DKR-MATCH-FUNC-80049794-COMBINED-REGISTER-PROBE`
- Summary: No new source match landed. This pass rejected one ordinary-C `func_80049794` combined register-allocation probe while keeping the function active.
...
- `./diff.sh -o func_80049794 -s --compress-matching 3 --no-pager` -> baseline promoted C remains `CURRENT (2550)`; combining `register f32 var_f20` with `register f32 var_f14` also leaves `CURRENT (2550)`, nonmatching
...
- Step: Continue with selector-recommended `func_80049794`, but do not repeat the rejected allocation, wave-scan, early-zero, `spA3`, or combined-register probes above. Next useful angle is a source-shape change that affects live ranges before the first `var_f20` assignment without increasing wave-scan register pressure, or a narrower investigation of why target keeps the early zero in `$f14` while current uses `$f16`. `func_80059208` remains a valid close alternate, but do not repeat the rejected final-offset expression/load-order probes above, including inline-X-only or `pad3` checkpoint-dot routing. Accept only after `gmake -j4 CROSS=tools/binutils/mips64-elf-` reports `Verify: OK`. Keep `func_80049794`, `func_80059208`, `trackbg_render_flashy`, and `func_8002B0F4` active rather than parked.
