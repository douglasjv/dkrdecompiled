...
- Branch: `master`
- HEAD: `41833d38`
- Completed task: `func_80059208-child-evidence-import`
- Summary: Imported `func_80059208` child evidence after the child committed a durable negative-evidence result.
...
- No child lane is active after the `func_80059208` evidence import; create
  exactly one next child only after refreshed routing.
...
- Task: `select next single child lane after func_80059208 evidence import`
- Packet class: `parent_child_select`
...
- Step: Refresh `python3 tools/query_goal_state.py next --compact --refresh`
  and `python3 tools/query_goal_state.py tooling`, then create exactly one
  child for a bounded function/family that is not `func_8002B0F4`,
  `func_80049794`, or `func_80059208` unless the packet names a genuinely
  distinct mechanism. Parent integration accepts only source-level C after
