...
- Generated at: 2026-05-14 23:44:50Z
- Branch: `master`
- HEAD: `8ec7d595`
- Completed task: `DKR-MATCH-ACTIVE-NONPARK-PROBES`
- Summary: No new source match landed. This pass kept the close functions active,
  recorded one missed `func_80049794` buoyancy constant probe, and rejected two
  ordinary-C `func_80059208` final-offset expression probes.
...
- `./diff.sh -o func_80059208 -s --compress-matching 3 --no-pager` -> baseline promoted C remains `CURRENT (870)`; reusing `diffY` for the checkpoint dot worsens to `CURRENT (1465)`, and positive `pad2` with `diffX = (pad2 - pad) / divisor` worsens to `CURRENT (1200)`, nonmatching
- Prior in-progress probe: `func_80049794` buoyancy constants as `-1.0f`/`10.0f` compiled but left focused score unchanged at `CURRENT (2550)`, nonmatching
