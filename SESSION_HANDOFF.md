...
- Generated at: 2026-05-14 22:50:32Z
- Branch: `master`
- HEAD: `0fca7695`
- Completed task: `DKR-MATCH-FUNC-80059208-PROBE-PASS`
- Summary: No new source match landed. This pass rejected several additional ordinary-C `func_80059208` lifetime/expression probes while keeping the function active.
...
- Step: Keep `func_80059208` active because the remaining diff is localized and object-level expected output exists for `racer.c`, but do not repeat the rejected final-offset probes above. Next useful angle is earlier lifetime/register pressure around the normalized `diffX`/`diffZ` stack slots (`0x54`/`0x4C`) or deliberately switch back to selector-recommended `func_80049794`. Accept only after `gmake -j4 CROSS=tools/binutils/mips64-elf-` reports `Verify: OK`. Keep `func_80049794`, `trackbg_render_flashy`, and `func_8002B0F4` active rather than parked.
