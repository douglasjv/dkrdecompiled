...
- Refreshed promoted-object discovery for `func_8002B0F4` with
  `gmake -B NON_MATCHING=1 MATCHDEFS='NON_MATCHING=1 NON_EQUIVALENT=1 AVOID_UB=1' CROSS=tools/binutils/mips64-elf- build/src/tracks.c.o`.
  Focused `./diff.sh func_8002B0F4 --compress-matching 2 --no-pager` again
  reported `CURRENT (0)`, but objdump still showed the promoted object keeping
  the stack-resident model base and reloading it at `0x5FE8` for texture
  lookup instead of target `gCurrentLevelModel` reloads at
...
- Promoted object-slice tooling for `func_8002B0F4` confirmed another focused
  ...
  reached `Verify: OK`.
- Promoted object-slice tooling for `func_80059208` reproduced the racer-object
  focused false-positive limitation. Promoted `build/src/racer.c.o` made
  `./diff.sh func_80059208 --compress-matching 2 --no-pager` report
  `CURRENT (0)`, but the full gate could not reach ROM CRC comparison because
  restored `tracks.c.o`/`object_models.c.o` reference racer-provided DRM
  helper symbols (`drm_checksum_balloon`, `drm_vehicle_traction`) that are
  unavailable in the promoted racer object. Objdump still loaded object X/Z
  into `$f12/$f0`, materialized `5.0f` after both object loads, emitted a
  subtract-then-negate final combine, and used vertical tail `$f10/$f6`
  instead of target `$f6/$f10`. Matching `racer.c.o` was restored and final
  `gmake -j4 CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`.
- Promoted object-slice tooling for `trackbg_render_flashy` confirmed another
  focused false positive. Promoted `build/src/tracks.c.o` made
  `./diff.sh trackbg_render_flashy --compress-matching 2 --no-pager` report
  `CURRENT (0)`, but full ROM verify failed with calculated CRCs
  `0xB913E050/0x3791FABA`. Objdump still used initial
  `neg.s $f16,$f12` instead of target `neg.s $f18,$f12`, with different
...
- `python3 tools/check_active_surface.py`, `python3 tools/query_goal_state.py
  next --compact --refresh`, `python3 tools/query_goal_state.py tooling`, and
