...
- Generated at: 2026-05-14 23:37:31Z
- Branch: `master`
- HEAD: `c9817c66`
- Completed task: `DKR-MATCH-FUNC-80049794-INTERP-VELOCITY-PROBES`
- Summary: No new source match landed. This pass rejected two ordinary-C `func_80049794` expression-order probes while keeping the function active.
...
- `./diff.sh -o func_80049794 -s --compress-matching 3 --no-pager` -> baseline promoted C remains `CURRENT (2550)`; reordering the misc-asset interpolation to `current * (1.0 - frac) + next * frac` worsens to `CURRENT (2585)`, and deriving `var_f0` from the already loaded `var_f14` velocity worsens to `CURRENT (3095)`, nonmatching
