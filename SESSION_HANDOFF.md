...
- Generated at: 2026-05-23 04:57:27Z
- Branch: `master`
- HEAD: `3ccc273b`
- Completed task: `func_80049794`
- Summary: Current-baseline grounded boss throttle/brake condition-order probe (`racer->velocity > -6.0 && racer->vehicleID >= VEHICLE_BOSSES`) missed; object-only focused diff first printed stale `CURRENT (0)`, full verify failed with calculated CRCs `0x5FDDE03F/0xC5DC283B`, and relinked focused diff regressed to `CURRENT (3555)`. The probe did not recover target `$f20/$f21` prologue saves, kept early zero in `$f16` instead of `$f14`, and left the wave loop reversed as current `a0`-bound/`v1`-loop instead of target `v1`-bound/`a0`-loop. Source was restored.
