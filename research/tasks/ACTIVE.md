...
- Discovery selector: `python3 tools/query_goal_state.py discovery` now recommends `trackbg_render_flashy` as `kind=mechanism_hypothesis`, with `func_80059208` demoted to `kind=tooling_first`, `func_8002B0F4` demoted to `kind=tooling_first`, and `func_80049794` demoted to `kind=fallback_note`.
- Latest validation baseline: `python3 tools/check_active_surface.py`, `python3 tools/query_goal_state.py next --compact --refresh`, `python3 tools/query_goal_state.py discovery`, `python3 -m py_compile tools/query_goal_state.py`, `gmake -j4 CROSS=tools/binutils/mips64-elf-`, and `./score.sh -s` passed on 2026-05-31 after the `func_80059208` evidence update and discovery-kind classifier refinement. Matching build reached `Verify: OK`; score remained 97.30%.
