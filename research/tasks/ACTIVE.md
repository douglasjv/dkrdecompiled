...
- Latest tooling note: `tools/query_goal_state.py` now applies recent revival cooldowns in shared routing state. `list --json --include-exhausted` no longer recommends parked candidates that `revival` classifies as recent cooldown, and JSON candidates expose `revival_cooldown: true` for the three parked revival misses.
...
- Current routing status after this note: all 4 default-routable guarded candidates have cooldown sidecars and all 3 parked candidates have recent revival cooldown. `python3 tools/query_goal_state.py next --compact --refresh` reports `recommended_next: discovery`; `python3 tools/query_goal_state.py list --json --include-exhausted` reports `recommended_next: null`.
- Discovery selector: `python3 tools/query_goal_state.py discovery` reports `discovery_next: tooling` when all sidecar candidates are cooldown/tooling-first or fallback status. `python3 tools/query_goal_state.py revival` reports `revival_next: tooling` because all parked candidates have recent revival cooldown evidence, including `init_particle_buffers` initial-only colour-tag local (`CURRENT (0)` focused, full verify CRCs `0xEF29C961/0xF604264D`).
- Latest validation baseline: `python3 tools/check_active_surface.py`, `python3 tools/query_goal_state.py next --compact --refresh`, `python3 tools/query_goal_state.py discovery`, `python3 tools/query_goal_state.py revival`, `python3 tools/query_goal_state.py list --json --include-exhausted`, `python3 -m py_compile tools/query_goal_state.py`, `gmake -j4 CROSS=tools/binutils/mips64-elf-`, and `./score.sh -s` passed on 2026-05-31 after the include-exhausted revival-cooldown routing fix. Matching build reached `Verify: OK`; score remained 97.30%.
