...
- Discovery selector: `python3 tools/query_goal_state.py discovery` now ranks sidecar guidance. Treat `func_8002B0F4` as cooldown/saturated until a distinct model-load lifetime/register-allocation mechanism is named; otherwise run discovery/tooling rather than another spelling or local-index probe.
- Latest validation baseline: `python3 tools/check_active_surface.py`, `gmake -j4 CROSS=tools/binutils/mips64-elf-`, and `./score.sh -s` passed on 2026-05-31 after the latest restored evidence update. Matching build reached `Verify: OK`; score remained 97.30%.
