...
- Latest selector-packet note: `func_80049794` remains active after a
  2026-05-25 promoted pointer-cursor wave-scan spelling missed. The source
  changed the `NON_EQUIVALENT` guard to `#if 1`, added
  `WaterProperties **wavePtr`, initialized `var_v1 = gRacerWaveCount - 1`,
  `var_a0 = var_v1`, and `wavePtr = &gRacerCurrentWave[var_a0]`, then
  decremented both `var_a0` and `wavePtr` in the wave scan while leaving the
  later `gRacerCurrentWave[var_a0 + 1]` uses unchanged. Pre-build `./diff.sh
  func_80049794 --compress-matching 2 --no-pager` misleadingly reported
  `CURRENT (0)`, but full verify failed with calculated CRCs
  `0xED20BE1F/0xDF1B0F4F`; relinked `./diff.sh func_80049794
  --compress-matching 2 --no-pager` regressed to `CURRENT (8562)`. The diff did
  get pointer decrement in the wave loop and avoided the target-exit array
  reload, but it still placed the decrement index in current `v1` instead of
  target `a0`, shifted the frame to `0x100`, and still lacked target
  `$f20/$f21` prologue saves. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok. Do not repeat this pointer-cursor wave-scan spelling; next
  `func_80049794` hypothesis needs an independent saved-FPR lifetime/wave
  allocation source shape, or pivot to another live candidate.
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  2026-05-25 promoted local `levelModel` pointer pressure spelling missed. The
  source changed the `NON_EQUIVALENT` guard to `#if 1`, added a local
  `LevelModel *levelModel`, assigned it from `gCurrentLevelModel`, and used it
  for segment, bounding-box, texture, and bottom segment accesses. Pre-build
  `./diff.sh func_8002B0F4 --compress-matching 2 --no-pager` misleadingly
  reported `CURRENT (0)`, but full verify failed with calculated CRCs
  `0x8632BBD2/0x4CA9FD95`; relinked `./diff.sh func_8002B0F4
  --compress-matching 2 --no-pager` regressed to `CURRENT (4208)`. The frame
  grew from target `0x128` to current `0x130`, the model pointer spilled at
  `0x12c(sp)`, the saved-register/prologue schedule shifted broadly, and the
  source still did not recover target fresh model loads or bottom
  population/sort. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok. Do not repeat this local `LevelModel *levelModel` pointer
  pressure spelling; next `func_8002B0F4` hypothesis needs a distinct early
  model-load pressure shape, or pivot to another live candidate.
- Latest alternate-packet note: `trackbg_render_flashy` remains active after a
...
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  ...
  bottom population loop from `wave = &D_8011D128[var_v0]; gTrackWaves[var_v0]
  = wave;` to `gTrackWaves[var_v0] = &D_8011D128[var_v0];`. Pre-build
  `./diff.sh func_8002B0F4 --compress-matching 2 --no-pager` misleadingly
  reported `CURRENT (0)`, but full verify failed with calculated CRCs
  `0x7D5E118A/0xB0A43A55`; relinked `./diff.sh func_8002B0F4
  --compress-matching 2 --no-pager` reported `CURRENT (2855)`. The direct
  population spelling still inserted the unwanted early `gCurrentLevelModel`
  spill at `0x60(sp)`, rotated the outer segment/grid register family, and
  shifted bottom population/sort labels. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, and `./score.sh -s`
  remained 97.30%. Do not repeat this direct bottom `gTrackWaves` population
  spelling; next `func_8002B0F4` hypothesis needs a distinct early model-load
  pressure shape or a pivot to another live candidate.
- Latest alternate-packet note: `trackbg_render_flashy` remains active after a
...
- Latest alternate-packet note: `trackbg_render_flashy` remains active after a
  ...
  `Verify: OK`, `./score.sh -s` remained 97.30%, and `python3
  tools/check_active_surface.py` reported active surface ok. Do not repeat this
  `gSPPolygon` final-flag macro spelling; next `trackbg_render_flashy`
  hypothesis needs a distinct early FPR allocation fix, or pivot to another
  routable packet.
- Latest worker-packet note: `func_8002B0F4` remains active after a
  2026-05-24 forked-worker declaration-order model-spill nudge missed. The
  worker promoted the guarded C and swapped local declaration order so
  `LevelModelSegmentBoundingBox *currentBoundingBox` preceded
  `LevelModelSegment *currentSegment`. Worker full verify failed with
  calculated CRCs `0x7856718A/0x66208CAA`; relinked `./diff.sh func_8002B0F4
  --compress-matching 2 --no-pager` stayed at `CURRENT (2860)`. The unwanted
  early `gCurrentLevelModel` load/spill to `0x60(sp)` before the segment loop
  remained, with broad segment/grid/tail register drift. Worker source was
  restored with no patch applied here; main checkout validation remained
  `Verify: OK`. Do not repeat this pointer declaration-order lifetime nudge;
  next `func_8002B0F4` hypothesis needs a distinct early model-load pressure
  shape that avoids segment-index carriers, pointer-add rewrites, texture-index
  temp carriers, grid predicate/order changes, and bottom-tail variants.
- Latest alternate-packet note: `func_80059208` remains active after a
...
- Latest alternate-packet note: `func_80059208` remains active after a
  ...
  missed. The source changed the `NON_MATCHING` guard to `#if 1` and rewrote
  only `if (racer->courseCheckpoint > -0x7D00)` as
  `if (-0x7D00 < racer->courseCheckpoint)`. Pre-build
  `./diff.sh func_80059208 --compress-matching 2 --no-pager` misleadingly
  ...
  `CURRENT (870)`. The spelling produced no movement in the upper-half
  courseCheckpoint branch and retained the final object-dot/checkpoint-dot
  plus vertical FPR drift. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this courseCheckpoint constant-left guard spelling.
  Next `func_80059208` hypothesis should avoid saturated upper-half decrement,
  courseCheckpoint guard, alternate-route guard, and final-tail microvariants
  unless paired with a distinct spline dataflow or final-tail allocation fix.
- Latest selector-packet note: `func_80049794` remains active after a
...
- Latest alternate-packet note: `trackbg_render_flashy` remains active after a
  ...
  `D_800DC92C` cursor loop to emit two triangles per iteration using
  `var_v0_3[0..5]`, then advance `var_v0_3` by 6 and `tris` by 2. Pre-build
  `./diff.sh trackbg_render_flashy --compress-matching 2 --no-pager`
  misleadingly reported `CURRENT (0)`, but full verify failed with calculated
  CRCs `0x938938F5/0x42F392C4`; relinked `./diff.sh trackbg_render_flashy
  --compress-matching 2 --no-pager` worsened to `CURRENT (1883)`. The body
  resembled the target two-triangle cursor shape, but the loop
  counter/register family became current `a1 += 1` with limit 4 instead of
  target `a2 += 2` with limit 8, and the early negative-cosine FPR drift
  remained. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this cursor-based two-triangle tail-loop spelling.
  Next `trackbg_render_flashy` hypothesis should avoid cursor two-triangle
  tail loops, direct-index two-triangle unroll, final triangle
  postincrement/indexed-table spellings, and saturated negative-cosine/
  first-ring FPR carrier variants unless paired with a distinct early FPR
  allocation fix, or pivot to another bounded routable packet.
- Latest selector-packet note: `func_80049794` remains active after a
...
- Latest alternate-packet note: `func_80059208` remains active after a
  ...
  calculated CRCs `0x53D141DF/0xB9D4B481`; relinked `./diff.sh func_80059208
  --compress-matching 2 --no-pager` stayed at `CURRENT (870)`. The lower clamp
  produced no useful movement, and the diff retained the same final
  object-dot/checkpoint-dot plus vertical FPR tail drift. Source was restored,
  `gmake -j4 CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`,
  `./score.sh -s` remained 97.30%, and `python3 tools/check_active_surface.py`
  reported active surface ok; do not repeat this `splinePos` lower-clamp
  constant-left spelling. Next hypothesis should avoid `func_80059208`
  lower-clamp constant-left, pre-fill counter expression/guard, fill-loop,
  upper-half decrement/alternate-route, normalization, wrong-way counter, final
...
- Latest alternate-packet note: `func_80059208` remains active after a
  ...
  `./diff.sh func_80059208 --compress-matching 2 --no-pager` misleadingly
  reported `CURRENT (0)`, but full verify failed with calculated CRCs
  `0x53905373/0x65198BEE`; relinked `./diff.sh func_80059208
  --compress-matching 2 --no-pager` worsened to `CURRENT (1515)`. The diff
  changed the control-point fill-loop pointer/index schedule from target
  `sltu` pointer-compare ordering to a `bne` loop, then carried FPR drift into
  the final object/checkpoint-dot and vertical tail. Source was restored,
  `gmake -j4 CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`,
  `./score.sh -s` remained 97.30%, and `python3 tools/check_active_surface.py`
  reported active surface ok; do not repeat this spline fill-loop plain-bound
  spelling. Next hypothesis should avoid `func_80059208` fill-loop,
  upper-half decrement/alternate-route, normalization-boolean, final
  object-dot, and final-tail clamp/negation microvariants unless paired with a
  distinct spline dataflow fix, or pivot to another bounded routable packet.
- Latest alternate-packet note: `func_8002B0F4` remains active after a
...
- Latest parked-packet revisit note: `init_particle_buffers` remains parked
  ...
  changed the `NON_MATCHING` guard to `#if 1` and removed only the unused
  `s32 pad` local. Full verify failed with calculated CRCs
  `0xC451FA69/0x4D796BC5`; relinked `./diff.sh init_particle_buffers
  --compress-matching 2 --no-pager` worsened to `CURRENT (2176)`. The frame
  shrank from target `0x68` to `0x60`, shifted parameter stack slots, and kept
  the saved-register allocation family wrong (`s3/s1/s7/s2/s4`-style count
  carriers instead of target `s1/s3/s7/s4/s8`, allocator tag not in target
  `s2`). Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this declaration-only dead-`pad` removal.
- Latest alternate-packet note: `func_80059208` remains active after a
  2026-05-24 promoted unit-vector normalization store-order spelling missed.
  The source changed the `NON_MATCHING` guard to `#if 1` and rewrote only the
  guarded normalization block from `diffX *= scale; diffZ *= scale;` to
  `diffZ *= scale; diffX *= scale;`. Full verify failed with calculated CRCs
  `0x53D141DF/0x16A3391F`; relinked `./diff.sh func_80059208
  --compress-matching 2 --no-pager` worsened from the promoted baseline to
  `CURRENT (942)`. The probe inverted the target normalization load/multiply
  and store order at `0x5a168`-`0x5a184`, while retaining the known final
  object/checkpoint-dot plus vertical FPR tail drift around `0x5a260`. Source
  was restored, `gmake -j4 CROSS=tools/binutils/mips64-elf-` reached
  `Verify: OK`, `./score.sh -s` remained 97.30%, and `python3
  tools/check_active_surface.py` reported active surface ok; do not repeat
  this unit-vector normalization store-order spelling.
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  2026-05-24 promoted triangle-hit predicate operand-order spelling missed.
  The source changed the `NON_EQUIVALENT` guard to `#if 1` and rewrote only
  `if (temp_ra_1 == temp_ra_2 && temp_ra_2 != temp_ra_3)` as
  `if (temp_ra_2 != temp_ra_3 && temp_ra_1 == temp_ra_2)`. Full verify
  failed with calculated CRCs `0xC349F192/0x2D70F23E`; relinked
  `./diff.sh func_8002B0F4 --compress-matching 2 --no-pager` worsened to
  `CURRENT (3635)`. The diff retained the known unwanted early
...
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  ...
  `currentBatch->textureIndex` through existing local `temp` before
  `surface = gCurrentLevelModel->textures[temp].surfaceType`; a sibling nested
  water-surface guard spelling on top produced no additional movement. Full
  verify failed for both shapes with calculated CRCs
  `0x7C4CE18A/0x3A298210`; relinked `./diff.sh func_8002B0F4
  --compress-matching 2 --no-pager` improved versus the promoted baseline
  family to `CURRENT (2435)` but still inserted the unwanted early
  `gCurrentLevelModel` spill at `0x60(sp)` and kept broad grid/sort register
  drift. Source was restored, `gmake -j4 CROSS=tools/binutils/mips64-elf-`
  reached `Verify: OK`, `./score.sh -s` remained 97.30%, and `python3
  tools/check_active_surface.py` reported active surface ok; do not repeat the
  current-stack texture-index carrier alone or with only nested water-surface
  guards.
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  2026-05-24 promoted early `sp108` zero-bang guard spelling missed. The
  source changed the `NON_EQUIVALENT` guard to `#if 1` and rewrote only
  `if (sp108 == 0 || sp108 >= 8)` as `if (!sp108 || sp108 >= 8)`. Full verify
  failed with calculated CRCs `0x7856718A/0x66208CAA`; relinked
  `./diff.sh func_8002B0F4 --compress-matching 2 --no-pager` stayed at
  `CURRENT (2860)`. The diff inserted the known unwanted early
  `gCurrentLevelModel` spill at `0x60(sp)` and kept the broader
  segment/grid/tail register drift. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this early `sp108` zero-bang guard spelling.
- Latest alternate-packet note: `func_80059208` remains active after a
  2026-05-24 promoted pre-fill counter negative-guard spelling missed. The
  source changed the `NON_MATCHING` guard to `#if 1` and rewrote only
  `if (counter < 0)` after `counter = racer->nextCheckpoint - 2` to
  `if (counter <= -1)`. Full verify failed with the promoted-baseline
  calculated CRCs `0x53D141DF/0xB9D4B481`; relinked `./diff.sh
  func_80059208 --compress-matching 2 --no-pager` stayed at `CURRENT (870)`.
  The pre-fill counter block produced no focused movement, and the diff
  retained the final object/checkpoint-dot plus vertical FPR tail drift around
  `0x5a260`. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this pre-fill counter negative-guard spelling.
- Latest alternate-packet note: `func_80059208` remains active after a
  2026-05-24 promoted early rewind threshold comparison-order spelling
  missed. The source changed the `NON_MATCHING` guard to `#if 1` and rewrote
  only `if (splinePos < -0.2)` as `if (-0.2 > splinePos)`. Full verify failed
  with the promoted-baseline calculated CRCs `0x53D141DF/0xB9D4B481`;
  relinked `./diff.sh func_80059208 --compress-matching 2 --no-pager` stayed
  at `CURRENT (870)`. The early threshold block was unchanged in the focused
  window, and the diff retained the final object/checkpoint-dot plus vertical
  FPR tail drift around `0x5a260`. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this early rewind threshold comparison-order
  spelling.
- Latest alternate-packet note: `trackbg_render_flashy` remains active after a
...
- Latest alternate-packet note: `func_8002B0F4` remains active after a
  2026-05-24 promoted batch-loop bound spelling missed. The source changed the
  `NON_EQUIVALENT` guard to `#if 1` and rewrote only
  `for (batchNum = 0; batchNum < currentSegment->numberOfBatches; batchNum++)`
  as `batchNum != currentSegment->numberOfBatches`. Full verify failed with
  calculated CRCs `0x6816700E/0x1D54A605`; relinked `./diff.sh func_8002B0F4
  --compress-matching 2 --no-pager` worsened to `CURRENT (3280)`. The diff
  changed the batch-loop branch from target-like `slt`/`bnez` to `bne`, kept
  the unwanted early `gCurrentLevelModel` spill at `0x60(sp)`, and broadened
  grid/tail drift. Source was restored, `gmake -j4
  CROSS=tools/binutils/mips64-elf-` reached `Verify: OK`, `./score.sh -s`
  remained 97.30%, and `python3 tools/check_active_surface.py` reported active
  surface ok; do not repeat this batch-loop `!= numberOfBatches` bound
  spelling.
- Latest alternate-packet note: `func_8002B0F4` remains active after a
