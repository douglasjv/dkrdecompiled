...
- Discovery selector: `python3 tools/query_goal_state.py discovery` now has all sidecar candidates in cooldown/tooling-first or fallback status; run discovery/tooling unless a distinct compiler-mechanism packet is named first.
- Latest validation baseline: `python3 tools/check_active_surface.py`, `python3 tools/query_goal_state.py next --compact --refresh`, `python3 tools/query_goal_state.py discovery`, `gmake -j4 CROSS=tools/binutils/mips64-elf-`, and `./score.sh -s` passed on 2026-05-31 after the `trackbg_render_flashy` evidence update. Matching build reached `Verify: OK`; score remained 97.30%.
