...
- Discovery selector: `python3 tools/query_goal_state.py discovery` now reports `discovery_next: tooling` when all sidecar candidates are cooldown/tooling-first or fallback status. `python3 tools/query_goal_state.py revival` intentionally includes parked candidates, cools down candidates with a 2026-05-31 revival-worker miss, and now recommends `func_80017A18` with its `research/tasks/PARKED.md` evidence.
- Latest validation baseline: `python3 tools/check_active_surface.py`, `python3 tools/query_goal_state.py next --compact --refresh`, `python3 tools/query_goal_state.py discovery`, `python3 tools/query_goal_state.py revival`, `python3 -m py_compile tools/query_goal_state.py`, `gmake -j4 CROSS=tools/binutils/mips64-elf-`, and `./score.sh -s` passed on 2026-05-31 after the revival tooling update. Matching build reached `Verify: OK`; score remained 97.30%.
