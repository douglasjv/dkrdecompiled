...
Next useful work needs discovery/tooling or a distinct model-load lifetime/register-allocation mechanism that predicts target-like in-loop `gCurrentLevelModel` loads or removal of the model stack spill, not another ordering, condition, literal, local-index, or carrier spelling.
